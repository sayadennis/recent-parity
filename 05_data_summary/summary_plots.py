...
fn = "FrequencyAndResultsO_DATA_2022-12-09_1014.csv"
...
for par_cat in data.parity_category.unique():
    ...
    ax.scatter(
        par_data.clin_tumor_size, par_data.tumor_size, label=par_cat, alpha=0.5
    )  # .map(colors)
...
for i in range(node.shape[0]):
    for j in range(node.shape[1]):
        text = ax.text(j, i, node.iloc[i, j], ha="center", va="center", color="w")
