...
means = [
    float(
        data.iloc[data["main_category"].values == varname, :][category]
        .values[0]
        .split()[0]
    )
...
for item, label in zip(
    lines, ["Overall", "Nulliparous", "<5 years", "5-10 years", ">=10 years"]
):
...
for category in ["Nulliparous", "<5 years", "5-10 years", ">=10 years"]:
    cts[category] = [
        float(
            data.iloc[data["sub_category"].values == bm, :][category]
            .values[0]
            .split()[0]
        )
...
# Comment the below chunk if we want a stacked bar graph (of counts)
# as opposed to a stacked percentage chart
pcts = {}
for key, _ in cts.items():
    pcts[key] = []
    for i in range(len(cts["Nulliparous"])):
        pcts[key].append(
            100 * cts[key][i] / np.sum([xval[i] for x, xval in cts.items()])
        )
...
ax[1, 0].bar(
    ...
    bottom=[
        np.sum([x, y, z])
        for x, y, z in zip(pcts["Nulliparous"], pcts["<5 years"], pcts["5-10 years"])
    ],
    label=">=10 years",
...
ax[1, 0].set_xticklabels(
    ["ER/PR+ HER2-", "HER2+", "Triple Negative"], rotation=45, ha="right"
)
# ax[1,0].legend()
        ...
        np.sum(
            [
                ...
                for x in data.iloc[
                    np.array([x in varnames for x in data["sub_category"].values])
                    & np.array(
                        [data["main_category"].values == "histologic_grade"]
                    ).ravel(),
                    :,
                ][category].values
            ]
