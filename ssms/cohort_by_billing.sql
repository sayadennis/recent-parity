...
SELECT DISTINCT p.ir_id, p.west_mrn AS EPIC_mrn, p.nmh_mrn AS Powerchart_mrn, p.first_name, p.last_name, p.birth_date --, pro.procedure_code
...
--		AND con.revoked_consent = 0
	...
	JOIN fact.vw_billing_transaction bt
		ON p.patient_key = bt.patient_key
	JOIN dim.vw_procedure pro
		ON bt.procedure_key = pro.procedure_key
		WHERE pro.procedure_code IN ('96040', '99242', '99243', '99244');
