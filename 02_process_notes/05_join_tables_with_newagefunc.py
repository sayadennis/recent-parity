...
demo = pd.read_csv(os.path.join(dn, 'patient_demographic.csv'), header=None, names=colnames)
gen = pd.read_csv(os.path.join(dn, 'formatted_genetic_testing_results_newagefunc.csv'))
par = pd.read_csv(os.path.join(dn, 'formatted_parity_results_newagefunc.csv'))

gen_region = pd.read_csv(os.path.join(dn, 'formatted_genetic_testing_results_regions_newagefunc.csv'))
...
joined = joined.iloc[joined['age_at_diagnosis'].values<=51,:]

oldagefunc_joined = pd.read_csv(os.path.join(dn, 'recent_parity_combined_data.csv'))

new_irid = []
for ir_id in list(joined['ir_id'].unique()):
    if ir_id not in list(oldagefunc_joined['ir_id'].unique()):
        new_irid.append(ir_id)

new = joined.iloc[[x in new_irid for x in joined['ir_id'].values],:]
new.to_csv(os.path.join(dn, 'newagefunction_addition_recent_parity_combined_data.csv'), index=False, header=True)
